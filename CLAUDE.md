...
│  · 지수 수급 차트(uPlot · 원장 전체) + 섹터 종목 그리드   │
│  · 신호 이력 · OOS 원장 (판정 아님 — 표본 축적의 가시화)  │
...
```jsonc
{
  ...
  "sector": {
    "name": "5개 산업 대표주",          // universe.json 의 label
    "stocks": [
      {
        "ticker":      "005930",
        "name":        "삼성전자",
        "industry":    "반도체",        // universe.json 의 industry (2026-08-19 추가)
        "foreign":     [-380, ...],        // length 30, 억원
        "institution": [210, ...],         // length 30, 억원
        "fhold":       [5310, 5308, ...],  // 외인지분율 × 100 (정수). 5310 = 53.10%
        "close":       [71200, 70800, ...] // 종가 (원, 정수, 양수)
      }
      // ... universe.json 의 종목 수만큼 (2026-08-19 기준 10종목 / 5산업)
    ]
  }
}
```
...
- 종목 `industry`는 **산업 라벨**이다(2026-08-19 추가). 유니버스가 단일 섹터(반도체 5종목)에서
  5개 산업 10종목으로 넓어지면서 들어왔다. **왜 산업인가:** 추적 종목의 일별 수익률 쌍상관을
  재 보니 산업 **내부** ρ=0.880, 산업 **간** ρ=0.309였다. 유효표본수 `N/(1+(N-1)ρ)` 기준으로
  같은 산업 안에서는 종목을 몇 개로 늘려도 하루 관측이 1.1개에서 안 움직이고, 산업을 나누자
  2.30개가 됐다(2.06배). **표본을 늘리는 유일한 축은 산업 분산이다.**
  종목 구성·선정 규칙·리밸런싱 원칙은 `universe.json` 이 정본이다.
- 종목 `close`는 **종가(원)**다(2026-08-17 추가). collector는 예전부터 이 값을 받아 억원
  환산에만 쓰고 버렸다 — 그래서 대시보드가 수급은 보여주면서 "그래서 올랐나"에는 답하지
  못했다. 수급만 있고 결과가 없는 화면이었다. 추가 호출은 없다(같은 응답에 이미 들어 있다).
  `_validate`가 0 이하를 차단한다 — 파싱이 실패하면 수익률이 조용히 망가지기 때문이다.
  **과거는 백필할 수 없다**(발행본 스냅샷에 종목 종가가 없다). 오늘부터 쌓인다.
- `as_of`는 **모든 피드가 공통 관측한** 가장 최근 확정 거래일이다(`common_window()`). 어느 피드가
  늦게 게시되면 `as_of`가 하루 물러난다 — 이는 정상 동작이지 지연이 아니다.
- `collected_at`은 **수집이 실제로 돈 시각**이다(2026-08-07 추가). `as_of`만으로는 "기준일이 어제인 것"이
...
**실제로 터진 함정 (2026-08-07 · 재발 금지)**

1. **장중 캔들을 종가로 오인** — 네이버 dayCandle/trend는 장이 열려 있는 동안에도 '오늘' 행을 준다.
   그 값은 그 시점까지의 부분 집계일 뿐이다. GitHub Actions 정시 큐 혼잡으로 수집이 매번 54~71분
   밀려 개장(09:00)선을 넘나든 결과, 30회 중 11회가 장중에 수집됐고 외국인 순매수가 최대 19.6배
   차이났다(2026-08-04은 부호 반전). → cron을 **장 마감 후(20:00 KST)** 로 옮기고,
   `collector.py`의 `_pending_session_date()` 가드가 마감 확정 전 세션을 계약에서 차단한다.
   가드는 `collector/guard.test.py`로 못박혀 있고 `run_daily.sh` 0단계에서 먼저 돈다.
2. **종목 시계열을 지수 날짜와 '위치로' 짝지음** — 지수 차트와 종목 trend는 별개 엔드포인트라
   최신 행이 같은 세션이라는 보장이 없다. 실제로 한 세션씩 어긋나 있었다(67거래일 중 64일 불일치).
   → 이제 `_row_date()`로 **날짜 기준** 정렬하고, `common_window()`가 모든 피드가 공통으로 관측한
   거래일만 창에 넣는다. 어느 피드가 늦게 게시되면 `as_of`가 하루 물러날 뿐, 어긋난 값은 안 나간다.
3. **원장 선착순 적재** — `appendIfNew`가 무조건 선착순이라 장중 스냅샷이 먼저 박히면 익일 확정치가
   기각됐다(6거래일 결손). → 확정 표식(`date_aligned`)이 없는 레코드는 확정본으로 승격한다.
   2026-08-07 이전 30건은 복원 불가로 판정해 격리했다(`backtest/audit_ledger.js` 참조).
...
- [x] GitHub Actions cron 등록, **한국시간 평일 20:00 (마감·피드 게시 후) 1회** — 개장 전 실행은 금지(§7 함정)
...
### 11.4 종합·추천 엔진 (`dashboard/research_digest.js`)

단순 링크 나열을 넘어, 리포트 목록(`window.RESEARCH`)과 수급 신호(`SIGNALS`)를 교차해 ① 일단위 종합과 ② 정독 추천을 만든다. 신호 엔진과 같은 순수함수·단위테스트 패턴(`research_digest.test.js`).

- **일단위 종합**: 카테고리 분포, 도메인 테마 키워드 빈도(오늘의 화두), 리포트 톤(긍정/부정 키워드 빈도 근사), 그리고 "수급 신호가 뜬 종목에 마침 리포트가 나왔는가"의 교차 서술. 서술은 집계값으로 **템플릿 생성**(AI 아님).
- **정독 추천**: 설명 가능한 점수로 상위 3건을 고르고 *왜*를 `reasons[]`로 노출한다. 핵심 가중치는 **수급 신호 교차**(Critical +80 / Alert +65) — 사실(수급)과 해석(리서치)이 겹치는 지점이 1순위. 그 외 섹터연관(+20)·시황(+15)·조회수·요약/목표가 유무.
- 추천을 "AI가 골랐다"고 하지 않는다. 규칙이 투명하고 이유가 보이므로 블랙박스보다 신뢰 가능하다.

### 11.5 향후 확장 (옵션)

- 추출형을 넘어선 **LLM 요약**: Alert 이상 신호 종목의 리포트만 Claude API로 3줄 다이제스트(별도 키·비용 합의 필요).
- Sprint 4 자동화에 합류: 수급 collector와 함께 데일리 배치, 텔레그램 다이제스트에 정독 추천 1~2건 링크 첨부.
