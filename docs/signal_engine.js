    ...
    data.sector.stocks.forEach(function (s) {
      ...
      if (Math.abs(zf) >= STOCK_MIN_Z) {
        ...
        sigs.push({
          tier: tier,
          scope: s.name,
          // 티커를 신호에 싣는다. 원장은 종목명으로만 기록돼 왔는데, 유니버스에서 종목이
          // 빠지면 이름→티커 매핑이 사라져 그 종목의 과거 신호를 정산할 수 없게 된다
          // (실제로 유니버스 확장 때 정산 4건 → 2건으로 조용히 줄었다).
          // 원장은 스스로 충분해야 한다 — 외부 매핑에 의존하면 증거가 휘발한다.
          ticker: s.ticker,
          z: Math.abs(zf),
          conf: conf,
          title:
